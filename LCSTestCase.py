...
class LCSTestCase:
    ...
    # numerical matrix. Uses an LCSMatrix's get_entry() method to retrieve
    # each entry.
    ...
    def execute(self, test_feedback):
        ...
        # Get the set of longest common substrings from the matrix
        ...
        message = (f'{"PASS" if passes else "FAIL"}: ' +
            f'"{self.string1}" and "{self.string2}"')
        if self.matrix_only:
            message += " (matrix generation only)"
